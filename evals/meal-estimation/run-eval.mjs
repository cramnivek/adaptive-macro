...
// `node --experimental-strip-types` (Node 22+) to load the TypeScript.
...
const PRECISE_TOLERANCE = 0.15;

/** Where a local Ollama server lives, when one is being tested. */
const OLLAMA_HOST = process.env.OLLAMA_HOST || 'http://127.0.0.1:11434';

// --- fill these in ----------------------------------------------------------

/** Sums a precise case's components into its reference macros. */
const referenceFrom = (components) =>
  components.reduce(
    (total, part) => ({
      kcal: total.kcal + (part.per100g.kcal * part.grams) / 100,
      proteinG: total.proteinG + (part.per100g.proteinG * part.grams) / 100,
      carbsG: total.carbsG + (part.per100g.carbsG * part.grams) / 100,
      fatG: total.fatG + (part.per100g.fatG * part.grams) / 100,
    }),
    { kcal: 0, proteinG: 0, carbsG: 0, fatG: 0 },
  );

/**
 * Cases come from cases.json. References for precise meals are computed here
 * from the per-100 g components rather than stored as totals, so a typo in a
 * component can never silently disagree with a hand-written total.
 */
async function loadCases() {
  ...
  return raw.map((c) => ({
    id: c.id,
    prompt: c.prompt,
    // tags[0] groups the report; tier is the cut that matters here.
    tags: [c.tier],
    tier: c.tier,
    expected: c.tier === 'precise' ? referenceFrom(c.components) : null,
    expectedRange: c.expectedRange ?? null,
    meta: { rangeBasis: c.rangeBasis ?? null },
  }));
}
...
async function runOllama(prompt, model) {
  ...
  const response = await fetch(OLLAMA_HOST + '/api/chat', {
    ...
    body: JSON.stringify({
      model,
      stream: false,
      // Ollama constrains generation to the schema, the same guarantee the
  ...
  // Validated against the same zod schema the app uses, so a local model that
  // returns plausible-looking but wrong-shaped JSON fails here rather than
  // being scored on a half-parsed object.
  const estimate = APP.MealEstimateSchema.parse(JSON.parse(text));
  ...
  return {
    ...
    raw: { stopReason: body.done_reason ?? null },
  };
}
...
/** Signed fractional error against a point reference. */
const signedError = (actual, reference) =>
  reference === 0 ? 0 : (actual - reference) / reference;

/**
 * Distance outside a range, as a fraction of the nearest bound.
 *
 * Anything inside the range scores zero. For a vague description there is no
 * single right answer, so any plausible figure is fully correct, and ranking
 * two in-range answers against each other would be inventing precision the
 * reference does not have.
 */
const rangeError = (actual, bounds) => {
  const [lo, hi] = bounds;
  if (actual >= lo && actual <= hi) return 0;
  return actual < lo ? (actual - lo) / lo : (actual - hi) / hi;
};
...
async function gradeCase(input, run, ref, ctx) {
  ...
  if (input.tier === 'precise') {
    const bias = signedError(got.kcal, input.expected.kcal);
    const errors = macros.map((k) => Math.abs(got[k] - input.expected[k]));
    return {
      grade: {
        kcal_ok: Math.abs(bias) <= PRECISE_TOLERANCE ? 1 : 0,
        kcal_err_pct: Math.abs(bias) * 100,
        kcal_bias_pct: bias * 100,
        macro_mae_g: errors.reduce((a, b) => a + b, 0) / errors.length,
      },
      explanation: {
        kcal_ok:
          'estimated ' + Math.round(got.kcal) + ' kcal against a reference of ' +
          Math.round(input.expected.kcal),
      },
    };
  }

  const bias = rangeError(got.kcal, input.expectedRange.kcal);
  const errors = macros.map((k) => {
    const [lo, hi] = input.expectedRange[k];
    if (got[k] >= lo && got[k] <= hi) return 0;
    return got[k] < lo ? lo - got[k] : got[k] - hi;
  });
  ...
  return {
    ...
    explanation: {
      kcal_ok:
        'estimated ' + Math.round(got.kcal) + ' kcal against a plausible range of ' +
        input.expectedRange.kcal[0] + '-' + input.expectedRange.kcal[1],
    },
  };
}

/** How many items it split the meal into — a model that returns one blob is doing something different. */
function perfFrom(run) {
  return { items: run.estimate?.items?.length ?? 0 };
}
